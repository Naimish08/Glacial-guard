  ...
  return (
    <div className="w-80 bg-card border-r border-border p-4 overflow-y-auto">
      <div className="mb-4">
        ...
        <p className="text-sm text-muted-foreground">AI-powered risk predictions</p>
      </div>

      <div className="space-y-3">
        {alerts.map((alert) => (
          <Card key={alert.id} className="p-3 shadow-card hover:shadow-soft transition-shadow">
            {/* Alert Header */}
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center space-x-2">
                <span className="text-lg">{getRiskIcon(alert.risk)}</span>
                <div>
                  <h3 className="text-sm font-semibold text-foreground">{alert.location}</h3>
            ...
            <div className="space-y-2">
              <div className="bg-muted/50 rounded-md p-2">
                <p className="text-xs font-medium text-foreground mb-1">📊 AI Analysis</p>
              ...
              <div className="bg-accent/10 rounded-md p-2">
                ...
                <p className="text-xs text-muted-foreground">{alert.shap}</p>
              </div>

              <div className="flex items-center justify-between text-xs">
                <div>
                  <span className="text-muted-foreground">📅 Forecast: </span>
                  <span className="font-medium text-foreground">{alert.forecast}</span>
                </div>
              ...
              <div className="pt-1">
                <p className="text-xs text-muted-foreground mb-1">🏘️ Affected Villages:</p>
                <div className="flex flex-wrap gap-1">
                  {alert.villages.map((village) => (
                    <Badge key={village} variant="outline" className="text-xs px-2 py-0.5">
                      {village}
                    </Badge>
                  ))}
                </div>
      ...
      <div className="mt-4 p-3 bg-gradient-glacier rounded-lg">
        <div className="text-center">
          <p className="text-sm font-semibold text-card-foreground">Today's Summary</p>
