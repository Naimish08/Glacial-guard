...
interface SidePanelProps {
  ...
  onClose: () => void;
  onViewLocation?: (coordinates: [number, number]) => void; // New prop
}
...
export const SidePanel = ({ location, onClose, onViewLocation }: SidePanelProps) => {
  ...
  try {
    ...
    console.warn('Map context not available');
  }
  ...
  useEffect(() => {
    if (!location) {
      console.error('Location prop is required for SidePanel');
    }
    if (!onClose) {
      console.error('onClose prop is required for SidePanel');
    }
  }, [location, onClose]);

  const handleViewOnMap = () => {
    if (location.coordinates && onViewLocation) {
